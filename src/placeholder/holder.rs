...
lazy_static! {
    pub static ref HOLDERS: Vec<Holder> = vec![
        ...
        create_holder!("commit:c", |s, _c| s.commit_count.to_string(), |s, _c, l|  update_padding_style(&s.commit_count.to_string(), l.map(|i| i.commit_count), Some(&STYLE.commit_hash))),
        create_holder!("author:e", |s, _c| s.author_email.clone(),     |s, _c, l|  update_padding_style(&s.author_email, l.map(|i| i.author_email), Some(&STYLE.author_email))),
        create_holder!("author:n", |s, _c| s.author_name.clone(),      |s, _c, l|  update_padding_style(&s.author_name, l.map(|i| i.author_name), Some(&STYLE.author_name))),
        create_holder!("time:r",   |s, _c| s.relative_time.clone(),    |s, _c, l|  update_padding_style(&s.relative_time, l.map(|i| i.relative_time), Some(&STYLE.relative_time))),
        create_holder!("time:a",   |s, _c| s.absolute_time.clone(),    |s, _c, l|  update_padding_style(&s.absolute_time, l.map(|i| i.absolute_time), Some(&STYLE.absolute_time))),

        // No specific custom styles or paddings needed for basic dirty text flags
        create_holder!("dirty",    |s, _c| s.dirty.clone(),            |s, _c, _l| update_padding_style(&s.dirty, None, None)),
        ...
        create_holder!(
            ...
            |s, caps, _l| {
                ...
                update_padding_style(&s.commit_hash[..target_len], None, Some(&STYLE.commit_hash))
            }
        ),
    ];
}
